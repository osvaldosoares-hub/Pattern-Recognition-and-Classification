...
def load_wdbc():
    possible = [
        os.path.join(THIS_DIR, 'wdbc.data'),
        os.path.join(TRAB6_DIR, '..', 'trabalho5', 'breast_cancer', 'wdbc.data'),
        os.path.join(TRAB6_DIR, '..', 'trabalho4', 'breast_cancer', 'wdbc.data'),
        os.path.join(TRAB6_DIR, '..', 'trabalho2', 'Breast Cancer', 'breast+cancer+wisconsin+diagnostic', 'wdbc.data')
    ]
    ...
    if data_path is None:
        raise FileNotFoundError("Arquivo wdbc.data não encontrado.")

    df = pd.read_csv(data_path, header=None)
    y_raw = df.iloc[:, 1].values  # M / B
...
def main():
    X, y, class_names, data_path = load_wdbc()
    ...
    with open('resultado.txt', 'w', encoding='utf-8') as f:
        f.write("TRABALHO 6 - BREAST CANCER - PARZEN BAYES\n")
        f.write("=" * 60 + "\n")
