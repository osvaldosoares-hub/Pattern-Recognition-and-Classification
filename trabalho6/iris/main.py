...
F1, F2 = 2, 3  # petal length x petal width
...
def plot_decision_surface_full(clf, X_train, y_train, X_test, y_test, filename):
    ...
    ax.set_title('Superfície de decisão - Parzen-Bayes (Iris)', fontweight='bold')
    ax.grid(alpha=0.3)
...
def main():
    ...
    with open('resultado.txt', 'w', encoding='utf-8') as f:
        f.write("TRABALHO 6 - IRIS - PARZEN BAYES\n")
        f.write("=" * 60 + "\n\n")
    ...
    plot_decision_surface_full(clf_plot, best_parzen['X_train'], best_parzen['y_train'],
                               best_parzen['X_test'], best_parzen['y_test'], 'superficies_decisao.png')
