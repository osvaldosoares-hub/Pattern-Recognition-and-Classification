...
def plot_decision_surface(X_train, y_train, X_test, y_test, clf, title, filename):
    ...
    for c in np.unique(y_train):
        m = (y_train == c)
        ax.scatter(
            X_train[m, F1], X_train[m, F2],
            marker='o', s=55, alpha=0.80, edgecolors='k',
    ...
    for c in np.unique(y_test):
        m = (y_test == c)
        ax.scatter(
            X_test[m, F1], X_test[m, F2],
            marker='^', s=70, alpha=0.95, edgecolors='k',
...
def plot_reference_style(X, y, filename='grafico_referencia.png'):
    ...
    ax.plot([-0.2, 5.0], [4.3, -0.5], color='gray', lw=1.2, alpha=0.85)  # diagonal semelhante ao quadro
...
def main():
    ...
    # novo gráfico no estilo do desenho enviado
    ...
    for real in range(N_REAL):
        ...
        for name, clf in classifiers.items():
            ...
            if name == 'GMM-Bayes' and acc > best_gmm['accuracy']:
                margem = acc - max([v for k, v in local_acc.items() if k != 'GMM-Bayes'] + [0.0])
                best_gmm.update({
                    'accuracy': acc,
                    'realizacao': real + 1,
                    ...
                    'y_pred': y_pred.copy(),
                    'justificativa': (
                        f"Realização {real+1} escolhida por apresentar a maior acurácia do GMM-Bayes "
                        f"({acc:.4f}). Margem parcial observada para os demais métodos: {margem:.4f}."
                    )
                })

    # Salvar resultado textual
    with open('resultado.txt', 'w', encoding='utf-8') as f:
        f.write("TRABALHO 5 - ARTIFICIAL I - GMM-BAYES\n")
        f.write("=" * 60 + "\n\n")
    ...
    # Superfície de decisão + treino/teste
    clf_plot = GMMBayesFull(n_components=3, reg_covar=1e-6, random_state=42)
    clf_plot.fit(best_gmm['X_train'], best_gmm['y_train'])
    plot_decision_surface(
        best_gmm['X_train'], best_gmm['y_train'],
        best_gmm['X_test'], best_gmm['y_test'],
        clf_plot,
        'Superfície de decisão - GMM-Bayes (Artificial I)',
