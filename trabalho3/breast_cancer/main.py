"""Classificador Naive Bayes, KNN e DMC - Breast Cancer Wisconsin (WDBC)
Dataset: wdbc.data  (569 amostras, 30 atributos, 2 classes: M/B).
Superfície de decisão e gaussianas: radius_mean (f0) x texture_mean (f1).
"""
...
TEST_SIZE = 0.2
KNN_K     = 5
F1, F2    = 0, 1    # radius_mean, texture_mean
    ...
    with open('wdbc.data', 'r') as f:
        for line in f:
            ...
            if not line:
                continue
            parts = line.split(',')
            # col 0 = ID, col 1 = class (M/B), cols 2-31 = features
            label = parts[1]
            feats = list(map(float, parts[2:32]))
...
def main():
    print("=" * 60)
    print("BREAST CANCER (WDBC)  —  Naive Bayes, KNN, DMC")
    print("=" * 60)
    ...
    print(f"Distribuicao: { {c: int((y==c).sum()) for c in classes} }\n")
    ...
    fig1.suptitle(f'Matrizes de Confusao — Breast Cancer  (Realizacao #{med+1})', fontsize=12)
    plot_cm(cm_nb,  cnames, f'Naive Bayes\nAcc={r["na"]*100:.1f}%',  axes[0])
    plot_cm(cm_knn, cnames, f'KNN (k={KNN_K})\nAcc={r["ka"]*100:.1f}%', axes[1])
    plot_cm(cm_dmc, cnames, f'DMC\nAcc={r["da"]*100:.1f}%',          axes[2])
    plt.tight_layout()
    ...
    fig2.suptitle(f'Superficies de Decisao — Breast Cancer\n'
    ...
    for ax, (cls_, kw, ttl) in zip(axes, [
        ...
        plot_decision_surface(cls_, kw, X, y, F1, F2,
                              r['tr'], r['te'], cnames, FEAT_NAMES, ax, ttl)
    plt.tight_layout()
    ...
    plot_gaussians(X, y, r['tr'], r['te'], F1, F2, FEAT_NAMES, cnames, ax,
                   f'Gaussianas NB por Classe — Breast Cancer\n'
