...
def plot_cm(cm, names, title, ax):
    ...
    ax.set_xticks(range(len(names))); ax.set_xticklabels(short, rotation=45, ha='right', fontsize=7)
...
def plot_decision_surface(clf_cls, clf_kw, X, y, f1, f2,
    ...
    for i, c in enumerate(classes):
        short = cnames[i-1][:10] if isinstance(cnames[i-1], str) else str(cnames[i-1])
        ...
        ax.scatter(X_te_n[y[te_idx]==c, 0], X_te_n[y[te_idx]==c, 1],
                   ...
                   linewidths=0.5, label=short)
    ax.set_xlabel(fnames[f1], fontsize=8); ax.set_ylabel(fnames[f2], fontsize=8)
    ax.set_title(title, fontsize=10); ax.legend(fontsize=6)
...
def load_data():
    ...
    arr = np.array(rows)
    # Last column is class label (1-6)
    y_raw = arr[:, 34].astype(int)

    # Columns 0-33 are features; column 33 (age) may contain '?'
    X_raw = arr[:, :34]
...
def main():
    print("=" * 60)
    print("DERMATOLOGY  —  Naive Bayes, KNN, DMC")
    print("=" * 60)
    ...
    fig1.suptitle(f'Matrizes de Confusao — Dermatology  (Realizacao #{med+1})', fontsize=12)
    ...
    fig2.suptitle(f'Superficies de Decisao — Dermatology\n'
    ...
    plot_gaussians(X, y, r['tr'], r['te'], F1, F2, FEAT_NAMES, cnames, ax,
                   f'Gaussianas NB por Classe — Dermatology\n'
