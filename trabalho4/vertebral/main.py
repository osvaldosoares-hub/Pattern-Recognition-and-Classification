...
def load_data():
    """Carrega dados da coluna vertebral."""
    data = []
    ...
    # Codificar labels
    label_map = {'Hernia': 'Hernia', 'Spondylolisthesis': 'Spondylolisthesis', 'Normal': 'Normal',
                 'DH': 'Hernia', 'SL': 'Spondylolisthesis', 'NO': 'Normal'}
    y_mapped = np.array([label_map.get(v, v) for v in y_raw])
    
    # Converter para números
    unique_labels = np.unique(y_mapped)
    label_to_num = {label: i for i, label in enumerate(unique_labels)}
    y = np.array([label_to_num[label] for label in y_mapped])
...
def plot_decision_surface(X_train, y_train, X_test, y_test, clf, title, filename, 
    ...
    for i in range(xx.shape[0]):
        for j in range(xx.shape[1]):
            ...
            # Expandir para 6 dimensões (features do vertebral)
            point_full = np.zeros((1, 6))
            point_full[0, [f1, f2]] = point_2d[0]
            # Para outros atributos, usar média dos dados de treinamento
            for k in range(6):
                if k not in [f1, f2]:
    ...
    for i, c in enumerate(np.unique(y_train)):
        ...
        ax.scatter(X_train_2d[mask_train, 0], X_train_2d[mask_train, 1],
                  marker='^', s=100, c=[colors[i]], label=f'Class {int(c)} (train)',
    ...
    for i, c in enumerate(np.unique(y_test)):
        ...
        ax.scatter(X_test_2d[mask_test, 0], X_test_2d[mask_test, 1],
                  marker='o', s=150, c=[colors[i]], label=f'Class {int(c)} (test)',
                  alpha=0.7, edgecolors='black', linewidth=1.5)
...
def main():
    ...
    try:
        ...
        print("ERRO: Arquivo 'column_3C.dat' não encontrado!")
        print("Certifique-se de que o arquivo está no diretório do script.")
        return
    
    print("=" * 80)
    print("ANÁLISE DE DISCRIMINANTES LINEARES E QUADRÁTICOS - VERTEBRAL DATASET")
    print("=" * 80)
    ...
    print(f"Distribuição: {dict(zip(np.unique(y), [sum(y==c) for c in np.unique(y)]))}")
    ...
    for real in range(N_REAL):
        ...
        if (real + 1) % 5 == 0:
            print(f"Realização {real + 1:2d}/20 - LDA: {acc_lda:.4f} | QDA: {acc_qda:.4f} | KNN: {acc_knn:.4f} | DMC: {acc_dmc:.4f}")
    ...
    plot_decision_surface(
        ...
        f'Superfície de Decisão - {best_realization["method"]} (Vertebral)\n{FEAT_NAMES[F1]} vs {FEAT_NAMES[F2]}',
