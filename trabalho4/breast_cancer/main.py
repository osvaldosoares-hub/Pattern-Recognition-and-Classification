...
class DMC:
    ...
    def predict(self, X):
        preds = []
        for x in X:
            d = {c: np.sqrt(((x - self.centroids_[c]) ** 2).sum())
...
def plot_decision_surface(X_train, y_train, X_test, y_test,
                          clf, title, filename, f1=F1, f2=F2):
    ...
    for i in range(xx.shape[0]):
        for j in range(xx.shape[1]):
            ...
            pt[f1]  = xx[i, j]
            pt[f2]  = yy[i, j]
            Z[i, j] = clf.predict(pt.reshape(1, -1))[0]
    ...
    for i, c in enumerate(np.unique(y_train)):
        ...
        ax.scatter(X_train_2d[mask, 0], X_train_2d[mask, 1],
                   marker='^', s=100, c=[plt.cm.viridis(i / 1)],
                   label=f'{CLASS_NAMES[int(c)]} (train)',
                   alpha=0.7, edgecolors='black', linewidth=1.5)
 
    for i, c in enumerate(np.unique(y_test)):
        mask = y_test == c
        ax.scatter(X_test_2d[mask, 0], X_test_2d[mask, 1],
                   marker='o', s=150, c=[plt.cm.viridis(i / 1)],
                   label=f'{CLASS_NAMES[int(c)]} (test)',
                   alpha=0.7, edgecolors='black', linewidth=1.5)
...
def main():
    ...
    # ── Tabela ordenada do melhor para o pior ─────────────────────────────────
    print("\n" + "=" * 80)
    ...
    plot_decision_surface(
        ...
        f'Superfície de Decisão - {best_realization["name"]} (Breast Cancer)\n'
