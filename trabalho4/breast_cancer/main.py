...
from quadratic_discriminant.qda import QuadraticDiscriminantAnalysis
...
def load_data():
    ...
    try:
        ...
        print("ERRO: Arquivo 'wdbc.data' não encontrado!")
        return None, None
    
    y_raw = np.array([r[0] for r in rows])
    X = np.array([r[1] for r in rows])
    
    # Converter labels (B -> 0, M -> 1)
    y = np.array([0 if label == 'B' else 1 for label in y_raw])
...
def plot_decision_surface(X_train, y_train, X_test, y_test, clf, title, filename,
    ...
    for i in range(xx.shape[0]):
        for j in range(xx.shape[1]):
            ...
            # Expandir para 30 dimensões
            point_full = np.zeros((1, 30))
    ...
    for i, c in enumerate(np.unique(y_train)):
        ...
        ax.scatter(X_train_2d[mask_train, 0], X_train_2d[mask_train, 1],
                  marker='^', s=100, c=[plt.cm.viridis(i/1)], label=f'{CLASS_NAMES[int(c)]} (train)',
    ...
    for i, c in enumerate(np.unique(y_test)):
        ...
        ax.scatter(X_test_2d[mask_test, 0], X_test_2d[mask_test, 1],
                  marker='o', s=150, c=[plt.cm.viridis(i/1)], label=f'{CLASS_NAMES[int(c)]} (test)',
                  alpha=0.7, edgecolors='black', linewidth=1.5)
...
def main():
    ...
    print("=" * 80)
    print("ANÁLISE DE DISCRIMINANTES LINEARES E QUADRÁTICOS - BREAST CANCER DATASET")
    print("=" * 80)
    print(f"\nDataset: Breast Cancer (WDBC)")
    print(f"Número de amostras: {X.shape[0]}")
    print(f"Número de atributos: {X.shape[1]}")
    print(f"Número de classes: {len(np.unique(y))}")
    print(f"Distribuição: Benign={sum(y==0)}, Malignant={sum(y==1)}")
    ...
    plot_decision_surface(
        ...
        f'Superfície de Decisão - {best_realization["method"]} (Breast Cancer)\n{FEAT_NAMES[F1]} vs {FEAT_NAMES[F2]}',
